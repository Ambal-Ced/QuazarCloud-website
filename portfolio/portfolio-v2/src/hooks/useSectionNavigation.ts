...
type ScrollableFlags = { showMoreOpen?: boolean; showMoreProjects?: boolean }

function isScrollableSection(current: number, flags: ScrollableFlags): boolean {
  const { showMoreOpen = false, showMoreProjects = false } = flags
  return (current === 1 && showMoreOpen) || (current === 2 && showMoreProjects)
}
...
export function useSectionNavigation(scrollContainerRef?: RefObject<HTMLElement | null>, flags: ScrollableFlags = {}) {
  ...
  }, [goNext, goPrev, current, scrollContainerRef, showMoreOpen, showMoreProjects])
